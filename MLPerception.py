...
with open('train_p.txt','r',encoding='utf-8') as f3:
    for p in f3.readlines():
        p=p.strip('\n')
        temp=list(map(eval,p[2:].split()))
        train_p.append(np.array(temp))

W1,b1=init_param(sigma,(12,100)),init_param(sigma,(12,1))
W2,b2=init_param(sigma,(12,12)),init_param(sigma,(12,1))
W3,b3=init_param(sigma,(12,12)),init_param(sigma,(12,1))
W4,b4=init_param(sigma,(4,12)),init_param(sigma,(4,1))
for gen in range(2):
    for i in range(train_w2v.shape[0]):
        ...
        sig4 = y4 - train_p[i].reshape(-1, 1)
        sig3 = np.dot(np.transpose(W4), sig4) * (np.gradient(np.tanh(z3), axis=0) + rd.uniform(0.05, 0.1))
        ...
        W4=W4-0.2*np.dot(sig4,y3.reshape(1,-1))
        b4=b4-0.2*sig4
...
data={'W1':W1,'b1':b1,'W2':W2,'b2':b2,'W3':W3,'b3':b3,'W4':W4,'b4':b4}
np.savez('train_para.npz',**data)
